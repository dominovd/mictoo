      ...
      sections={[
        {
          ...
          body: 'Mictoo не требует аккаунта и не собирает персональные данные. Когда вы загружаете файл, браузер отправляет его напрямую в Vercel Blob — хранилище файлов Vercel, размещённое в США. Файл лежит там короткое время, пока наш сервер не передаст его в Whisper API от OpenAI для распознавания, и удаляется из Vercel Blob автоматически сразу после завершения транскрипции — обычно меньше чем за минуту. Ежечасный задание-уборщик подметает загрузки, которые по какой-то причине не удалились сразу. Сам Mictoo не сохраняет постоянных копий вашего аудио, видео или расшифровки на собственных серверах.',
        },
        {
