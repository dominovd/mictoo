...
// Per-user concurrent queue depth. Authenticated users can have at most this
// many jobs in the 'queued' or 'processing' state at once. Stops a single
// user from monopolising the queue (or running up the bill if Groq is down).
// Anonymous users aren't tracked here — they're already capped by the IP
// rate limit and don't use the queue heavily.
export const PER_USER_QUEUE_LIMIT = 3
...
export class UserQueueFullError extends Error {
  constructor(currentDepth, limit) {
    super(`User has ${currentDepth} pending jobs (limit ${limit}).`)
...
function userActiveKey(userId) {
  return `mictoo:user:${userId}:active`
}
...
export async function enqueueJob(payload) {
  ...
  // Per-user depth check (authenticated users only).
  if (payload.userId) {
    const userDepth = await redis.scard(userActiveKey(payload.userId))
    if (userDepth >= PER_USER_QUEUE_LIMIT) {
      throw new UserQueueFullError(userDepth, PER_USER_QUEUE_LIMIT)
    }
  }
  ...
  // Track this job against the user's depth limit. Released in
  // releaseUserSlot() when the job ultimately completes or fails.
  if (payload.userId) {
    await redis.sadd(userActiveKey(payload.userId), jobId)
    await redis.expire(userActiveKey(payload.userId), JOB_TTL_SECONDS)
  }

  return jobId
}

// Called by the worker after setJobResult / setJobError when a job's
// terminal state is reached. Frees a slot in the user's depth budget so
// they can submit another file.
export async function releaseUserSlot(userId, jobId) {
  if (!userId || !jobId) return
  const redis = await getRedis()
  if (!redis) return
  await redis.srem(userActiveKey(userId), jobId)
}

// How many active (queued or processing) jobs the user currently has.
// Used by the client to decide whether the "Upload more" button is
// available without actually trying to enqueue and getting a 429.
export async function getUserActiveCount(userId) {
  if (!userId) return 0
  const redis = await getRedis()
  if (!redis) return 0
  return await redis.scard(userActiveKey(userId))
}
