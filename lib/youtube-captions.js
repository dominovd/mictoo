...
// Innertube API (the internal YouTube API yt-dlp uses). Each YouTube
// client (WEB, ANDROID, IOS, TVHTML5...) calls /youtubei/v1/player with
// its own public API key. These keys are baked into the YouTube apps
// themselves and are not secret — they grant no access to anything
// outside YouTube's own Innertube endpoint. However, GitHub's secret
// scanner flags any AIzaSy* string as a Google API key, so we read
// them from env vars instead of hardcoding. Set YT_INNERTUBE_KEY_WEB
// etc. in Vercel env to enable. With the YOUTUBE_URL_INPUT_ENABLED
// flag off (see components/UploadZone.js) this whole path is unused
// in production anyway — kept in tree for the 2026-06-24 revisit.
const INNERTUBE_KEYS = {
  WEB: process.env.YT_INNERTUBE_KEY_WEB || '',
  ANDROID: process.env.YT_INNERTUBE_KEY_ANDROID || '',
  IOS: process.env.YT_INNERTUBE_KEY_IOS || '',
  TVHTML5: process.env.YT_INNERTUBE_KEY_TVHTML5 || '',
}
...
const CLIENTS = {
  ...
  TVHTML5: {
    // Smart-TV embedded player, current version. Tends to bypass
    // bot-detection more aggressively than WEB because TVs can't be
    // asked to log in to confirm humanity.
    headers: {
  ...
  MWEB: {
    // Mobile-web player. Different bot-detection profile than desktop
    // WEB; sometimes serves captions when WEB refuses.
    headers: {
...
async function fetchPlayerViaInnertube(videoId, clientName = 'WEB') {
  ...
  if (!res.ok) {
    // Surface the HTTP status so the caller can distinguish "API key
    // wrong / bad request" (400) from "IP blocked at network layer"
    // (403/429) from "endpoint moved" (404).
...
async function fetchPlayerViaInnertubeChain(videoId, order = ['TVHTML5', 'MWEB', 'TVHTML5_EMBED', 'ANDROID', 'IOS', 'WEB']) {
  const attempts = []
  for (const client of order) {
    try {
      const player = await fetchPlayerViaInnertube(videoId, client)
      const status = player?.playabilityStatus?.status || null
      const reason = player?.playabilityStatus?.reason || null
      const tracks = player?.captions?.playerCaptionsTracklistRenderer?.captionTracks
      attempts.push({ client, status, reason, tracks: tracks?.length || 0 })
