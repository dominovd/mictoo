...
async function fetchFromTranscriptAPI(videoId) {
  ...
  // Their 422 = "no captions / video unavailable" (per docs). Treat as a
  // soft fail so caller can show the 'this video has no captions' UX.
  if (status === 422 || body?.error?.code === 'no_captions') {
    const e = new Error(body?.error?.message || 'Video has no captions.')
    e.code = 'NO_CAPTIONS'
    throw e
  }
  if (status === 429) {
