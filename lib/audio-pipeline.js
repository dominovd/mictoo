...
export async function extractAudio(buf, srcExt = 'mp4') {
  ...
  try {
    await run(ffmpegPath(), [
      ...
      '-i', input,
      '-vn',                // drop video
      '-c:a', 'aac',        // re-encode to AAC even if source was AAC, so the
                            // container is clean and the chunk timestamps are
                            // sane (copy can produce monotonic-timestamp errors
                            // on some inputs). At 128kbps cost is small.
      '-b:a', '128k',
      '-ar', '44100',
      '-ac', '2',
      '-movflags', '+faststart',
 ...
 *
 * Each chunk re-encodes to AAC@128k stereo in a fresh m4a container, so the
 * chunk's internal timestamps start at zero. The caller is responsible for
 * offsetting transcription timestamps by `startSec` at merge time.
 */
